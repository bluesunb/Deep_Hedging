    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for _ in range(gradient_steps):
            ...
            with th.no_grad():
                ...
                next_q2_values = F.relu(next_q2_values)
                target_q2_values = replay_data.rewards ** 2 + \
                                   (1 - replay_data.dones) * (self.gamma**2 * next_q2_values +
                                                              2*self.gamma*replay_data.rewards * next_q_values)

            # Get current Q-values estimates for each critic network
            current_q_values = self.critic(replay_data.observations, replay_data.actions)
            current_q2_values = self.critic2(replay_data.observations, replay_data.actions)
            current_q2_values = tuple(F.relu(current_q2) for current_q2 in current_q2_values)
            ...
            if self._n_updates % self.policy_delay == 0:
                ...
                std_cost_loss = th.abs(
                    F.relu(self.critic2.q1_forward(replay_data.observations, self.actor(replay_data.observations))) - \
