...
class SAC(OffPolicyAlgorithm):
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            critic2_loss = 0.5 * sum([F.mse_loss(current_q2, target_q2_values) for current_q2 in current_q2_values])
    ...
    def _excluded_save_params(self) -> List[str]:
        save_params = super(SAC, self)._excluded_save_params()
        return save_params + ["actor", "critic", "critic_target", "critic2", "critic2_target"]

    def _get_torch_save_params(self) -> Tuple[List[str], List[str]]:
        state_dicts = ["policy", "actor.optimizer", "critic.optimizer", "critic2.optimizer"]
        if self.ent_coef_optimizer is not None:
